<x-app-layout>
    ...
    <div class="modal hidden fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
        <div class="w-full max-w-lg bg-white shadow-lg rounded-lg p-8 relative">
            ...
            <div class="my-8 text-center">
                ...
                <div class="relative flex items-center my-2">
                    <input type="text" placeholder="Enter Icon Link" class="px-4 py-3 bg-white text-gray-800 w-full text-sm border border-gray-300 focus:border-blue-600 outline-none rounded-lg" other-icon-link="iconlink" />
                </div>
                <div class="relative flex items-center my-2">
                    <input type="text" placeholder="Enter Your Link" class="px-4 py-3 bg-white text-gray-800 w-full text-sm border border-gray-300 focus:border-blue-600 outline-none rounded-lg" other-acc-link="yourlink" />
                </div>

                <button onclick="addLinks()" type="button" class="px-5 py-2.5 !mt-8 w-full bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg tracking-wide">Add</button>
        </div>
    </div>
    ...
    <input type="text" name="other_links" id="otherLinksArray">
        <input type="submit" value="submit" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700">
    </x-form>

    <script>
        let otherLinksData = []; // Global array to store link objects
        ...
        function addLinks(){
            ...
            const iconLink = document.querySelector('input[other-icon-link="iconlink"]').value;
            const yourLink = document.querySelector('input[other-acc-link="yourlink"]').value;
            otherLinksContainer.innerHTML += `
                <div class="relative rounded-full border p-3 dark:border-white">
                    <i class="fa fa-solid fa-xmark absolute top-[-15px] right-0 dark:text-white cursor-pointer" onclick="otherIconRemove(this)"></i>
                    <a href="${yourLink}">
                        <img width="25px" src="${iconLink}" alt="">
                    </a>
                </div>
            `;

             // Add the link object to the global array
            otherLinksData.push({ iconLink, yourLink });

            modal.classList.add('hidden');
            document.querySelector('input[other-icon-link="iconlink"]').value = '';
            document.querySelector('input[other-acc-link="yourlink"]').value = '';
        }
